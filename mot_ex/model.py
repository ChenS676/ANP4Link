...
class FastJointDropEdgeNode(nn.Module):
    def __init__(self, p_edge: float = 0.2, alpha: float = 1.0, doscale: bool = True) -> None:
        super().__init__()
        self.drop_edge = DropAdj(dp=p_edge, doscale=doscale)
        self.alpha = alpha

    def forward(self, adj: SparseTensor) -> tuple[SparseTensor, torch.Tensor]:
        """
        Args:
            adj (SparseTensor): Input adjacency matrix.

        Returns:
            new_adj (SparseTensor): New SparseTensor after joint edge-node dropout.
            node_mask (BoolTensor): Mask indicating which nodes are kept.
        """
        # Step 1: Drop edges fast
        dropped_adj = self.drop_edge(adj)
        num_nodes = adj.size(0)
        # Step 2: Fast compute degrees without materializing adjacency
        rowptr = dropped_adj.storage.rowptr()
        degree = rowptr[1:] - rowptr[:-1]  # [N] degree vector
        # Step 3: Fast decide node dropout probability
        connected_mask = (degree > 0)  # BoolTensor
        # Build distance vector (0 if connected, inf if isolated)
        inf = 1e9
        distance = torch.full_like(degree, fill_value=inf, dtype=torch.float32)
        distance[connected_mask] = 0.0
        # Step 4: Compute per-node dropout probability
        prob_node_drop = 1.0 - torch.exp(-self.alpha * distance)
        # Step 5: Sample keep/drop mask
        keep_mask = torch.bernoulli(1 - prob_node_drop).bool()
        # Step 6: Mask edges fast via storage
        src, dst = dropped_adj.storage.row(), dropped_adj.storage.col()
        edge_keep = keep_mask[src] & keep_mask[dst]
        # Step 7: Efficiently select edges
        new_adj = torch_sparse.masked_select_nnz(dropped_adj, edge_keep, layout='coo')
        return new_adj, keep_mask
