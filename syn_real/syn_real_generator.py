...
from automorphism import run_wl_test_and_group_nodes
normalize_labels = lambda labels: (labels - np.mean(labels)) / np.std(labels)

def analyze_label_change_vs_degree(graph_before, graph_after, save_prefix='analysis'):
    """
    Analyze the correlation between WL label change and node degree.

    Args:
        graph_before (Data): PyG graph before perturbation.
        graph_after (Data): PyG graph after perturbation.
        save_prefix (str): File path prefix to save plots and results.

    Returns:
        correlation (float), pval (float), summary_df (pd.DataFrame)
    """
    G_after = to_networkx(graph_after, to_undirected=True)
    num_nodes = graph_after.num_nodes

    # Run WL test
    _, labels_before = run_wl_test_and_group_nodes(graph_before.edge_index, num_nodes=num_nodes, num_iterations=30)
    _, labels_after = run_wl_test_and_group_nodes(graph_after.edge_index, num_nodes=num_nodes, num_iterations=30)
    labels_before = np.array(labels_before)
    labels_after = np.array(labels_after)

    # Degree from perturbed graph
    deg = np.array([G_after.degree[n] for n in range(num_nodes)])

    # Label change indicator
    labels_before = normalize_labels(labels_before)
    labels_after = normalize_labels(labels_after)

    label_change = abs(labels_before - labels_after)
    label_change = normalize_labels(label_change)
    # Correlation analysis
    correlation = np.corrcoef(label_change, np.arange(num_nodes))[0, 1]
    print(f"Correlation between WL label change and degree: {correlation:.4f} (p={pval:.4g})")

    # Save as DataFrame
    summary_df = pd.DataFrame({
        'node_id': np.arange(num_nodes),
        'degree': deg,
        'wl_changed': label_change
    })
    summary_df.to_csv(f"{save_prefix}_label_vs_degree.csv", index=False)

    # Boxplot
    plt.figure()
    sns.boxplot(x=label_change, y=deg)
    plt.xticks([0, 1], ["WL Unchanged", "WL Changed"])
    plt.ylabel("Node Degree")
    plt.title("Node Degree vs. WL Label Change")
    plt.savefig(f"{save_prefix}_label_vs_degree_boxplot.png")
    plt.close()

    return correlation, pval, summary_df
