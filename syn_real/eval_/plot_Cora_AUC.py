...
#     loc="lower right",           # Changed to lower right where space is usually available
#     bbox_to_anchor=(0.98, 0.02), # Anchors the legend inside the bottom-right corner
#     ncol=2,                      # Kept 2 columns as requested
#     frameon=True,
#     framealpha=0.8,              # Increased alpha slightly for better readability over lines
#     fancybox=True,
#     borderaxespad=0.5            # Adds a small padding from the axis edge
# )

ax.set_xlabel(r"$EAR$", fontsize=LABEL_SIZE)
ax.set_ylabel("AUC (/%)", fontsize=LABEL_SIZE)
ax.set_xticks(new_alpha)
ymin = 93.0  # ablation 99.5
ymax = 100.02
...
plt.savefig(f"{base}/ablation_Exp1_Cora_SYN_AUC_Real.pdf")
