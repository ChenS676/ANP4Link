...
raw_citeseer_auc_data = [
    ...
    ("GCN-LAP",
     [0.9, 0.82, 0.72, 0.62, 0.52, 0.43, 0.35, 0.23, 0.19,  0.12, 0.06],
     [99.79, 99.65, 99.35, 99.04, 98.78, 98.23, 98.29, 97.81, 97.51, 97.16, 95.80],
     [0.07, 0.09, 0.19, 0.13, 0.14, 0.23, 0.29, 0.25, 0.24, 0.25, 0.45]),
    ...
    ("GCN-DW",
     [0.9, 0.82, 0.72, 0.62, 0.52, 0.43, 0.35, 0.23, 0.19,  0.12, 0.06],
     [99.86, 99.80, 99.60, 99.37, 99.09, 98.58, 98.45, 97.92, 97.83, 97.36, 96.23],
     [0.06, 0.08, 0.10, 0.12, 0.16, 0.23, 0.17, 0.20, 0.23, 0.24, 0.24]),

    ("GCN-RF",
     [0.9, 0.82, 0.72, 0.62, 0.52, 0.43, 0.35, 0.23, 0.19,  0.12, 0.06],
     [99.87, 99.69, 99.75, 99.60, 99.39, 99.27, 99.27, 98.46, 98.48, 98.16, 97.01],
     [0.04, 0.06, 0.06, 0.08, 0.09, 0.11, 0.13, 0.15, 0.19, 0.20, 0.37]),
    ...
    # ('Proposed w.o D2',
    #  [0.9, 0.82, 0.72, 0.62, 0.52, 0.43, 0.35, 0.23, 0.19, 0.12, 0.06],
    #  [99.9, 99.8, 99.79, 99.78, 99.75, 99.74, 99.73, 99.71, 99.7175, 99.6825, 99.67],
...
legend = ax.legend(
    ...
    loc="lower right",           # Changed to lower right to find white space
    bbox_to_anchor=(0.85, 0.05), # Anchors it 2% from the right and 5% from the bottom
    ncol=2,                      # Kept 2 columns for a balanced look
    frameon=True,
    framealpha=0.8,              # Higher alpha so data lines don't distract from text
    fancybox=True,
    borderaxespad=0.1            # Minimal padding to maximize plot area
)

# Optional: Ensure the legend frame is clearly visible
...
plt.savefig(f"{base}/ablation_Exp1_Citeseer_SYN_AUC_Real.pdf")