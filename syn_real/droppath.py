...
from torch_geometric.utils import (to_networkx, from_networkx, train_test_split_edges, to_undirected)
...
# -------------------------------------------------
# Stochastic depth / DropPath for node features
# -------------------------------------------------
def drop_path(x, keep_prob: float = 1.0, training: bool = False):
    """
    Stochastic depth / DropPath adapted for 2D node features [num_nodes, feat].
    If training and keep_prob < 1.0, randomly drops entire node representations
    (independently per node), scaling survivors by 1/keep_prob.
    """
    if keep_prob >= 1.0 or not training:
        return x
    # mask per node (shape [num_nodes, 1]) so all features of a node drop together
...
class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
                 ...
                 cat_node_feat_mf=False, data_name=None, sd_keep: float = 1.0):
        ...
        self.sd_keep = sd_keep  # stochastic depth keep prob (1.0 disables)
        self.invest = 1
    ...
    def forward(self, x, adj_t):
        if self.invest == 1:
            print('layers in gcn:', len(self.convs), '| sd_keep =', self.sd_keep)
            self.invest = 0
        ...
        for i, conv in enumerate(self.convs):
            ...
            if i < len(self.convs) - 1:
                x = F.relu(x)
            # DropPath then standard dropout
            x = drop_path(x, keep_prob=self.sd_keep, training=self.training)
...
def perturb_disjoint(graph_data, args, inter_ratio, intra_ratio, total_edges):
    ...
    N = 20
    _, updated_graph_data, star_edges = attach_star_graph_with_features(G, updated_graph_data, N, ig)
...
def load_real_world_graph(dataset_name="Cora"):
    if dataset_name in ['Cora', 'Citeseer', 'PubMed']:
        dataset = Planetoid(root='/tmp/' + dataset_name, name=device_dataset(dataset_name))
        data = dataset[0]
    elif dataset_name.startswith('ogbl'):
        # TODO: handle ogbl
        raise NotImplementedError("ogbl dataset loading not implemented in this script.")
    else:
        raise ValueError(f"Unknown dataset: {dataset_name}")
    return data


def device_dataset(name):
    # Planetoid expects proper capitalization (already provided), keep as-is
    return name
...
def parse_args():
    ...
    parser.add_argument('--pt_path', default=f"plots/Citeseer/processed_graph_inter0.5_intra0.5_edges1000_auto0.7200_norm1_0.7676.pt", type=str)
    # gnn setting
    ...
    # NEW: stochastic depth keep-prob (1.0 disables DropPath)
    parser.add_argument('--sd_keep', type=float, default=1.0,
                        help='Stochastic depth keep prob (1.0 = disabled)')
...
def run_training_pipeline(data, metrics, inter, intra, total_edges, args):
    ...
    model = eval(args.gnn_model)(
        ...
        data_name=args.data_name, sd_keep=args.sd_keep
    ).to(device)
...
def main():
    ...
    if not os.path.exists(f'plots/{args.data_name}'):
    ...
    if args.data_name == 'Cora':
        ...
        inter_ratios, intra_ratios, total_edges_list, multi_factor = [0.1], [0.5], [1], 1
...
# python /hkfs/work/workspace/scratch/cc7738-automorphism/ANP4Link/
# python syn_real/droppath.py --data_name Cora --gnn_model GCN --num_layers 3 --hidden_channels 128 --dropout 0.3 --sd_keep 0.9  --runs 